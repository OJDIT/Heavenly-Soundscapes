...
export default function UtilitiesPage() {
  ...
  const handleDeleteSpecificFile = async () => {
    if (
      confirm(
        "Are you sure you want to delete the file '1744684630167 Voice 001' from all storage locations?"
      )
    ) {
      setIsDeleting(true);
      setError(null);
  ...
  const handleClearAllAudio = async () => {
    if (
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to clear files: ${response.status} ${response.statusText}`
          );
        }
    ...
    try {
      ...
      if (storedContent) {
        ...
        const filteredContent = audioContent.filter((item: any) => {
          return !(
            (item.filename &&
              item.filename.includes("1744684630167 Voice 001")) ||
      ...
      <div className="container py-12 pt-24">
        ...
        <div className="border border-gold-500/20 rounded-lg p-6 mb-8">
          ...
          <p className="mb-4 text-muted-foreground">
            Use this utility to clear ALL audio files from your system. This
            will give you a clean slate to upload your own content.
          </p>
            ...
            <div className="mt-4 bg-green-500/10 border border-green-500/50 text-green-500 rounded-md p-3">
              ...
              <div className="text-sm space-y-1 mt-2">
                {result.clearedFromLocalStorage && (
                  <p>
                    ✓ Cleared {result.localStorageCount || "all"} files from
                    localStorage
                  </p>
                )}
                {result.clearedFromSupabase && (
                  <p>
                    ✓ Cleared {result.supabaseCount || "all"} files from
                    Supabase Storage
                  </p>
                )}
                {result.clearedFromBlob && (
                  <p>
                    ✓ Cleared {result.blobCount || "all"} files from Vercel Blob
                  </p>
                )}
              </div>
        ...
        <div className="border border-gold-500/20 rounded-lg p-6 mb-8">
          ...
          <p className="mb-4 text-muted-foreground">
            Use this utility to remove the problematic file "1744684630167 Voice
            001" from all storage locations.
          </p>
          ...
          {result && result.deletedFromLocalStorage !== undefined && (
            <div className="mt-4 bg-green-500/10 border border-green-500/50 text-green-500 rounded-md p-3">
              ...
              <div className="text-sm space-y-1 mt-2">
                {result.deletedFromLocalStorage && (
                  <p>✓ Removed from localStorage</p>
                )}
                {result.deletedFromSupabase && (
                  <p>✓ Removed from Supabase Storage</p>
                )}
                {result.deletedFromBlob && (
                  <p>✓ Potentially removed from Vercel Blob</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="border border-gold-500/20 rounded-lg p-6">
          ...
          <p className="mb-2 text-muted-foreground">
            If the automatic deletion doesn't work, you can try these manual
            steps:
          </p>
          ...
          <ol className="list-decimal pl-5 space-y-2 text-sm">
            ...
            <li>
              Look for any audio files with "1744684630167 Voice 001" in the
              title
            </li>
            <li>Delete these files using the delete button in the dashboard</li>
            <li>
              If the file still appears, try clearing your browser's
              localStorage:
