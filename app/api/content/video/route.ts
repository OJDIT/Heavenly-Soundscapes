...
export async function GET() {
  try {
    ...
    try {
      ...
      if (!initData.success) {
        console.warn(
          "Warning: Failed to initialize video-files bucket:",
          initData.error
        );
      }
    } catch (initError) {
    ...
    try {
      const { data: files, error } = await listFilesInBucket(
        "video-files",
        "videos"
      );
      ...
      if (files && files.length > 0) {
        ...
        const { data: thumbnails } = await listFilesInBucket(
          "video-files",
          "thumbnails"
        );
        ...
        const videoFiles = files.map((file) => {
          ...
          if (thumbnails) {
            ...
            const matchingThumbnail = thumbnails.find(
              (t) =>
                t.name.includes(fileBaseName) ||
                file.name.includes(t.name.split(".")[0])
            );

            if (matchingThumbnail) {
              thumbnailUrl = getFilePublicUrl(
                "video-files",
                matchingThumbnail.name
              );
            }
          }
          ...
          return {
            id: `video-${file.id || Date.now()}-${Math.random()
              .toString(36)
              .substring(2, 9)}`,
    ...
    const mockDataWithIds = mockVideoContent.map((item) => {
      if (!item.id) {
        return {
          ...item,
          id: `video-mock-${Date.now()}-${Math.random()
            .toString(36)
            .substring(2, 9)}`,
        };
      }
