  ...
  try {
    ...
    if (!bucket) {
      return NextResponse.json(
        { error: "Missing bucket parameter", success: false },
        { status: 400 }
      );
    }
    ...
    if (!supabase) {
      return NextResponse.json(
        { error: "Failed to initialize Supabase client", success: false },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      if (bucketError) {
        if (
          ...
          return NextResponse.json(
            { error: `Bucket error: ${bucketError.message}`, success: false },
            { status: 500 }
          );
        }
      }
